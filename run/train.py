...
from utils.utils import evaluate_dt
from Gato_models.model import DecisionTransformer
...
# ------------------------------------------------------------------------------------------------------------------
    ...
    save_model_path = os.path.join(output_dir, save_model_name)
    save_best_model_path = save_model_path[:-3] + "_best.pt"

    log_csv_name = prefix + "_log_" + start_time_str + ".csv"
    log_csv_path = os.path.join(output_dir, log_csv_name)
...
# ------------------------------------------------------------------------------------------------------------------
    ...
    env = gym.make(env_name)  # 用于评估的时候使用
...
# ------------------------------------------------------------------------------------------------------------------
    ...
    for i_train_iter in range(max_train_iters):
        ...
        results = evaluate_dt(model, device, context_len, env, rtg_target, rtg_scale,
